...
userRoute.post("/users/:userId", async (req, res, next) => {
  ...
  for (const bodyProp in req.body) {
    ...
    } else if (bodyProp === "badges") {
      for (const bProp in req.body.badges) {
        if (!validBadgeProps.includes(bProp)) {
          return res
            .status(400)
            .send(
              "users/ POST request formulated incorrectly." +
  ...
  try {
    ...
    if (thisUser) {
      ...
      thisUser = await new User({
        ...
        badges: {
          roundsPlayedBadge: req.body.badges.roundsPlayedBadge,
          fastTimeBadge: req.body.badges.fastTimeBadge,
          lowStrokesBadge: req.body.badges.lowStrokesBadge,
          streakBadge: req.body.badges.streakBadge,
          highScoreBadge: req.body.badges.highScoreBadge,
        },
        rounds: [],
      }).save();
      return res
  ...
  try {
    ...
    if (status.modifiedCount != 1) {
      ...
      res
        .status(404)
        .send(
          "Account not updated. Either no account with that id" +
    ...
    res
      .status(400)
      .send("Unexpected error occurred when updating user in database: " + err);
  }
});
...
userRoute.delete("/users/:userId", async (req, res, next) => {
  ...
  try {
    ...
    return res
      .status(400)
      .send(
        "Unexpected error occurred when attempting to delete" +
